<?php
	...
	// Consulta o banco
	$sql = 'SELECT COD_IBGE, NOME_MUNICIP, UF, PIB_2011 FROM municipios_2015 WHERE COD_IBGE LIKE ? OR NOME_MUNICIP LIKE ? OR UF LIKE ? OR PIB_2011 LIKE ?';
	...
	while ( $row = $db->fetch() ) {
	  	echo $row['COD_IBGE']. '#' .$row['NOME_MUNICIP']. '#' .$row['UF']. '#' .$row['PIB_2011'] . '#';
	}
?>